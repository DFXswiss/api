...
export class SupportPdfService {
  ...
  /**
   * The file record of a limit-request decision, the counterpart of the report the Google Sheet
   * uploaded for every final decision (`UserNotes` / `LimitRequestReport`). It states what was asked
   * for, what the account was allowed before and after, who decided, and the internal note — the same
   * fields the sheet carried, so an old and a new report read alike in the customer's file.
   */
  createLimitRequestPdf(userData: UserData, dto: GenerateLimitRequestPdfDto, generatedAt: Date): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      try {
        const pdf = new PDFDocument({ size: 'A4', margin: 50 });
        // Reject the promise on a PDFKit stream error after the synchronous draw block (the try/catch
        // around it only covers errors thrown before pdf.end()).
        ...
        const marginX = 50;
        const { width } = pdf.page;
        const formatLimit = (limit?: number): string => (limit != null ? `${limit.toLocaleString('de-CH')} CHF` : '-');

        pdf.moveDown(2);
        pdf.fontSize(18).font('Helvetica-Bold').fillColor('#072440');
        pdf.text('Limit Request Report', marginX);
        pdf.moveDown(0.5);
        pdf.fontSize(10).font('Helvetica').fillColor('#333333');
        pdf.text(`UserData ID: ${userData.id}`, marginX);
        pdf.text(`Datum: ${generatedAt.toISOString().split('T')[0]}`, marginX);
        pdf.moveDown(1);

        this.drawOnboardingSectionHeader(pdf, 'Kunde', marginX);
        this.drawOnboardingField(pdf, 'Name', userData.completeName ?? '-', marginX, width);
        this.drawOnboardingField(pdf, 'Verified Name', userData.verifiedName ?? '-', marginX, width);
        this.drawOnboardingField(pdf, 'Account Type', userData.accountType ?? '-', marginX, width);
        this.drawOnboardingField(pdf, 'KYC Level', String(userData.kycLevel ?? '-'), marginX, width);
        pdf.moveDown(0.5);

        this.drawOnboardingSectionHeader(pdf, 'Antrag', marginX);
        this.drawOnboardingField(pdf, 'Beantragtes Limit', formatLimit(dto.requestedLimit), marginX, width);
        this.drawOnboardingField(pdf, 'Mittelherkunft', dto.fundOrigin ?? '-', marginX, width);
        this.drawOnboardingField(pdf, 'Investitionszeitpunkt', dto.investmentDate ?? '-', marginX, width);
        pdf.moveDown(0.5);

        this.drawOnboardingSectionHeader(pdf, 'Entscheid', marginX);
        this.drawOnboardingField(pdf, 'Entscheid', dto.decision, marginX, width);
        this.drawOnboardingField(pdf, 'Bisheriges Jahreslimit', formatLimit(dto.previousLimit), marginX, width);
        this.drawOnboardingField(
          pdf,
          'Neues Jahreslimit',
          // No `??` fallback: the caller (SupportService.generateAndSaveLimitRequestPdf) guarantees before
          // ever reaching this renderer that a granting decision carries grantedLimit and a non-granting
          // one carries previousLimit — so exactly one of the two branches ever actually applies, and there
          // is nothing left to default.
          LimitRequestAccepted(dto.decision) ? formatLimit(dto.grantedLimit) : formatLimit(dto.previousLimit),
          marginX,
          width,
        );
        this.drawOnboardingField(pdf, 'Bearbeitet von', dto.clerk, marginX, width);
        pdf.moveDown(0.5);

        this.drawOnboardingSectionHeader(pdf, 'Interne Aktennotiz', marginX);
        pdf.fontSize(9).font('Helvetica').fillColor('#333333');
        pdf.text(dto.note?.trim() || '-', marginX, pdf.y, { width: width - marginX * 2 });

        pdf.moveDown(2);
        pdf.fontSize(8).font('Helvetica').fillColor('#999999');
        pdf.text(`Generated by DFX - ${generatedAt.toISOString()}`, marginX);
