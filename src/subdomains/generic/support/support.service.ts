...
import { Recommendation, RecommendationMethod } from '../user/models/recommendation/recommendation.entity';
...
export class SupportService {
  ...
  private async buildGraphPayload(
    ...
    const edges: RecommendationGraphEdge[] = recommendationEdges.map((r) => ({
      ...
      confirmationDate: r.confirmationDate,
      created: r.created,
      // only ref-code recommendations carry the code the user actually entered at signup (that pair's
      // standalone USED_REF edge is skipped below); for other methods the coincident usedRef is merely a
      // backfill from setRecommenderRefCode, never a code the user typed, so it must not be surfaced
      refCode:
        r.method === RecommendationMethod.REF_CODE
          ? refEdges.get(`${r.recommender.id}-${r.recommended.id}`)?.refCode
          : undefined,
    }));
