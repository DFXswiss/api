...
export class UserService {
  ...
  async updateUserAddress(userDataId: number, data: KycChangeAddressData, userId?: number): Promise<UserV2Dto> {
    const userData = await this.userDataRepo.findOne({
      where: { id: userDataId },
    ...
    await this.userDataService.updateUserAddress(userData, data.address);
