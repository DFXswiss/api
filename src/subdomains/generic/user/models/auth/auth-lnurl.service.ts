...
export class AuthLnUrlService {
  ...
  @DfxCron(CronExpression.EVERY_30_SECONDS, { scope: CronScope.BOTH, process: Process.LNURL_AUTH_CACHE })
  ...
  @DfxCron(CronExpression.EVERY_5_MINUTES, { scope: CronScope.BOTH, process: Process.LNURL_AUTH_CACHE })
  ...
  async login(signupDto: AuthLnurlSignupDto, userIp: string): Promise<AuthLnurlSignInResponseDto> {
    ...
    try {
      const verifyResult = secp256k1.verify(
        Util.stringToUint8(sig, 'hex'),
        Util.stringToUint8(k1, 'hex'),
        Util.stringToUint8(key, 'hex'),
      );
