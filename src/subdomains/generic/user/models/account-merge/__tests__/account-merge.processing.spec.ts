...
describe('AccountMerge processing state', () => {
  ...
  describe('executeMerge cleanup on failure', () => {
    ...
    it('completes the request when the committed merge returns after internally accounting for effect failures', async () => {
      ...
      // This request carries no reason, so it is not a mail merge: the compliance exemption must stay off.
      expect(mergeUserData).toHaveBeenCalledWith(master.id, slave.id, slave.mail, false, false);
