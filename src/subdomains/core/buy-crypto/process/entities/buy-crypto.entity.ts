...
export class BuyCrypto extends IEntity {
  ...
  isCheckoutTxAmlPass(blacklist: SpecialExternalBankAccount[]): boolean {
    return (
      this.target.asset.cardBuyable &&
      !blacklist.some((b) => b.iban && b.iban === this.checkoutTx.cardFingerPrint) &&
      (this.user.status !== UserStatus.NA || this.checkoutTx.ip === this.user.ip)
    );
  }
