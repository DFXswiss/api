...
describe('PreferUsdtOverBtcForLiquidityTrades migration (pg-mem semantics)', () => {
  ...
  it("down() rejects an audit entry naming a foreign action outside this migration's reachable rows", async () => {
    await seedBaselineChain(queryRunner, { withW: true });
    await insertAsset(queryRunner, 1, 'ETH');
    await insertRule(queryRunner, { id: 1, deficitStartActionId: BTC_ID, targetAssetId: 1 });
    ...
    // Foreign action: exists, but is none of the 17 pairs, no clone, and its own onFailId does not
    // point at any pair or clone id — genuinely outside this migration's reachable footprint.
    const FOREIGN_ID = 8888;
    await insertAction(queryRunner, {
      id: FOREIGN_ID,
      system: 'Foreign',
      command: 'noop',
    ...
    await expect(migration.down(queryRunner)).rejects.toThrow(
      new RegExp(`Audit entry id ${FOREIGN_ID} .* outside this migration's reachable rows`),
    );

    // Fail closed: nothing changed, including the foreign action itself.
    expect(await actionOnFail(queryRunner, W_ID)).toBe(afterUp.w);
    expect(await actionOnFail(queryRunner, USDT_ID)).toBe(afterUp.u);
    expect(await actionOnFail(queryRunner, BTC_ID)).toBe(afterUp.b);
    expect(await ruleStart(queryRunner, 1)).toBe(afterUp.rule);
    expect(await actionOnFail(queryRunner, FOREIGN_ID)).toBeNull();
    expect(await auditMessages(queryRunner)).toHaveLength(1);
  });
