...
// Also thrown by the BankTxConsumer forward path for a BUY_CRYPTO whose linked buy_crypto doAmlCheck will still
// price (BuyCrypto.isAmlPricingPending) and handled the same way by its forward-loop catch (verbose, cursor/watermark
// stays → head-of-line retry next run).
