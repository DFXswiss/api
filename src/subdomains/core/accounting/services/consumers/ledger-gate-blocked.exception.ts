...
// Also thrown by the BankTxConsumer forward path for a not-yet-priceable row (BUY_CRYPTO linked but not yet AML-priced; BUY_CRYPTO_RETURN not yet linked to its chargeback) and handled the same way by its
// forward-loop catch (verbose, cursor/watermark stays → head-of-line retry next run).
