...
export abstract class PurchaseStrategy extends PurchaseLiquidityStrategy {
    ...
    for (const prioritySwapAsset of await this.swapAssets()) {
      try {
        return {
          ...
          amount: await this.getSwapAmountForPurchase(referenceAsset, referenceAmount, targetAsset, prioritySwapAsset),
        };
      } catch (e) {
  ...
  private async calculateSwapAmountForPurchase(
    ...
    if (referenceAsset.id === targetAsset?.id) {
      const targetAmount = await this.dexService.getTargetAmount(
        swapAsset,
        swapAsset.minimalPriceReferenceAmount,
        referenceAsset,
      );
      const swapAmount = (referenceAmount * swapAsset.minimalPriceReferenceAmount) / targetAmount;

      // adding 5% cap to liquidity swap to cover meantime referenceAmount price difference (initially taken from Kraken/Binance)
      return Util.round(swapAmount * 1.05, 8);
    }
