...
import { PayInCardanoService } from 'src/subdomains/supporting/payin/services/payin-cardano.service';
...
export abstract class CardanoStrategy extends SendStrategy {
  ...
  async checkConfirmations(payIns: CryptoInput[], direction: PayInConfirmationType): Promise<void> {
    for (const payIn of payIns) {
      try {
        ...
        const isConfirmed = await this.payInCardanoService.checkTransactionCompletion(
  ...
  protected topUpCoin(payIn: CryptoInput, amount: number): Promise<string> {
    return this.payInCardanoService.sendNativeCoinFromDex(payIn.address.address, amount);
  }
}
