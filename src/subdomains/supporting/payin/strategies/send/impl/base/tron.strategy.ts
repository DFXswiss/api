...
import { PayInTronService } from 'src/subdomains/supporting/payin/services/payin-tron.service';
...
export abstract class TronStrategy extends SendStrategy {
  ...
  async checkConfirmations(payIns: CryptoInput[], direction: PayInConfirmationType): Promise<void> {
    for (const payIn of payIns) {
      try {
        ...
        const isConfirmed = await this.payInTronService.checkTransactionCompletion(
  ...
  protected topUpCoin(payIn: CryptoInput, amount: number): Promise<string> {
    return this.payInTronService.sendNativeCoinFromDex(payIn.address.address, amount);
  }
}
