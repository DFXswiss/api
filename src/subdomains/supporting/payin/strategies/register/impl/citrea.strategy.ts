...
@Injectable()
export class CitreaStrategy extends EvmStrategy implements OnModuleInit {
  protected readonly logger = new DfxLogger(CitreaStrategy);

  private static readonly LAST_PROCESSED_BLOCK_KEY = 'citreaLastProcessedBlock';
  private lastProcessedBlock: number | null = null;

  @Inject() private readonly settingService: SettingService;

  constructor(private readonly citreaService: PayInCitreaService) {
    super();
  }
  ...
  async checkPayInEntries(): Promise<void> {
    ...
    this.printInputLog(log, processedBlock ?? 'omitted', Blockchain.CITREA);
  }

  // --- HELPER METHODS --- //

  private async getNewEntriesWithBlock(): Promise<{ entries: PayInEntry[]; processedBlock: number | null }> {
    const currentBlock = await this.citreaService.getCurrentBlockNumber();
    ...
    this.logger.verbose(`Processing Citrea blocks ${fromBlock} to ${toBlock}`);

    const newEntries = await this.fetchTransactionsForBlockRange(fromBlock, toBlock);

    if (newEntries.length > 0) this.logger.info(`Found ${newEntries.length} new Citrea transactions`);
  ...
  private async fetchTransactionsForBlockRange(fromBlock: number, toBlock: number): Promise<PayInEntry[]> {
    ...
    for (const address of addressesToMonitor) {
      const [coinTransactions, tokenTransactions] = await this.citreaService.getHistory(address, fromBlock, toBlock);
  ...
  private async mapCoinTransactionsToEntries(
    ...
    const asset = await this.assetService.getCitreaCoin();

    return transactions.map((tx) => ({
      senderAddresses: tx.from,
      receiverAddress: BlockchainAddress.create(tx.to, Blockchain.CITREA),
      txId: tx.hash,
  ...
  private async loadPersistedState(): Promise<void> {
    const persistedBlock = await this.settingService.get(CitreaStrategy.LAST_PROCESSED_BLOCK_KEY);
  ...
  private async updateLastProcessedBlock(blockNumber: number): Promise<void> {
    await this.settingService.set(CitreaStrategy.LAST_PROCESSED_BLOCK_KEY, blockNumber.toString());
