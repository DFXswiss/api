...
@Injectable()
export class TronStrategy extends RegisterStrategy implements OnModuleInit {
  protected logger: DfxLogger = new DfxLogger(TronStrategy);

  private addressWebhookMessageQueue: QueueHandler;

  private tronPaymentDepositAddress: string;
  ...
  onModuleInit() {
    ...
    this.tronPaymentDepositAddress = TronUtil.createWallet({ seed: Config.payment.tronSeed, index: 0 }).address;
  }
  ...
  private async processWebhookTransactions(dto: TatumWebhookDto): Promise<void> {
    ...
    const ownWalletAddress = this.tronService.getWalletAddress();
  ...
  private async getPayInAddresses(): Promise<string[]> {
    ...
    addresses.push(this.tronPaymentDepositAddress);

    return addresses;
  }

  private mapTronTransaction(dto: TatumWebhookDto, supportedAssets: Asset[]): PayInEntry | undefined {
  ...
  private getTxType(address: string): PayInType | undefined {
    return Util.equalsIgnoreCase(this.tronPaymentDepositAddress, address) ? PayInType.PAYMENT : PayInType.DEPOSIT;
  }
