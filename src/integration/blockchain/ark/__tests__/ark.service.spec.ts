...
describe('ArkService', () => {
  ...
  // Derive ark address from public key
  const arkAddress = bech32m.encode('ark', bech32m.toWords(publicKey), 1024);
  ...
  const extendedAddress = bech32m.encode('ark', bech32m.toWords(extendedPayload), 1024);
  ...
  describe('verifySignature', () => {
    it('should verify a valid signature against a pubkey-derived address', async () => {
      const message = 'test message for ark verification';
      const signature = signMessage(message);

      const result = await service.verifySignature(message, arkAddress, signature);
    ...
    it('should reject a signature for a different message', async () => {
      ...
      const result = await service.verifySignature('different message', arkAddress, signature);
    ...
    it('should reject a signature for a different address', async () => {
      ...
      const otherAddress = bech32m.encode('ark', bech32m.toWords(otherPubKey), 1024);
    ...
    it('should return false for an invalid signature hex', async () => {
      const result = await service.verifySignature('test', arkAddress, 'not-valid-hex');

      expect(result).toBe(false);
    });

    it('should return false for an empty signature', async () => {
      const result = await service.verifySignature('test', arkAddress, '');
    ...
    it('should return false for a truncated signature', async () => {
      ...
      const result = await service.verifySignature('test', arkAddress, truncated);
    ...
    it('should verify signatures with different messages independently', async () => {
      ...
      expect(await service.verifySignature(message1, arkAddress, sig1)).toBe(true);
      expect(await service.verifySignature(message2, arkAddress, sig2)).toBe(true);

      // Cross-verify should fail
      expect(await service.verifySignature(message1, arkAddress, sig2)).toBe(false);
      expect(await service.verifySignature(message2, arkAddress, sig1)).toBe(false);
    });
  });
