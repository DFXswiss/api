...
- `Fee` field on `Trade` and `LastFee` on `ExecutionReport` are populated by
  ...
  commission is embedded in the quoted bid/ask spread (see Scrypt FAQ: _"The
  price you see is what you get"_). Implication: the implicit cost is not
...
- `AvgPx` doc string explicitly says _"Does not include fees"_ — confirming
  that for non-RFQ orders, Fee is meant to be a separate field even when
  the venue defaults it to zero.
- For `OrdType: RFQ`, `TradedPx` is described as _"all-in price that
  includes fees"_ — different semantics from Limit/Market, so any future
  RFQ integration needs its own pricing logic.
- **FIX-only `OrdType: A = LimitAllIn`** (FIX PDF, "New Order Single"
  section, Tag 40): _"requested price/size includes fees"_. This is **not
