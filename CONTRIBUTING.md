...
Both inventories (`docs/endpoints.md` and `docs/load-sites.md`) are produced by the scripts under `scripts/inventory/`. A full `npm run docs:inventory` run is a data source, not a replacement for the published documents — they carry passages adjusted by hand that the renderer does not produce. The run therefore leaves its output in a work directory and does not touch `docs/`; overwriting the published documents takes an explicit `--write-docs`. To keep the published `docs/load-sites.md` current, use the `apply_drift.py` workflow described in `scripts/inventory/README.md`; a PR that moves or adds load sites should update it that way rather than regenerating it. `docs/endpoints.md` has no such workflow and is maintained by hand, as the notes below describe.
