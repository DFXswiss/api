...
# How a query builder narrows its query, as printed in the Mechanism column. The keys are what
# extract-load-sites.py records; only the printed form belongs to the document.
SELECT_LABEL = {
    ...
    'projected-full-join': 'projected, full join',
}
...
# ---------------- load sites: drop the query builders that write ----------------
cache = {}
def is_write_qb(s):
...
t = ["# Database load sites", "",
     ...
     "",
     "| | Sites |", "| --- | ---: |",
     f"| `.select([...])` or `PROJECTION.apply(...)` — an explicit field list | **{sel.get('field-list', 0)}** |",
     f"| `.select('alias.column')` — names columns one by one | **{sel.get('named-columns', 0)}** |",
     f"| `.select('alias')` — selects the root alias, **loads every column** | {sel.get('alias-only', 0)} |",
     f"| no `select` at all — loads every column | {sel.get('no-select', 0)} |",
     f"| `getCount()` or `getExists()` — the select list is discarded, **no row is materialised** | "
     f"{sel.get('count-only', 0)} |",
     f"| projects, but a `leftJoinAndSelect` loads a relation whole | {sel.get('projected-full-join', 0)} |",
     "",
...
print(f"load sites: {len(loads)} ({writes} writing query builders removed) | "
      f"find {kinds['find']} / QB {kinds['query-builder']} / SQL {kinds['raw-sql']} | "
      f"field list {sel.get('field-list',0)}")
