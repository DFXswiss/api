...
| Coverage ratchet | `jest.coverage-gate.config.js` | 450 files, whole suite                     | Has coverage regressed anywhere it was already complete? |
...
Of the 450 pinned files, **256 carry real logic** (they have functions and/or branches) and
...
`jest.coverage-gate.config.js` holds the 450 paths in two arrays, `PINNED_LOGIC` (logic-carrying
