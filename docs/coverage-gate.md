...
| Coverage ratchet | `jest.coverage-gate.config.js` | 444 files, whole suite                     | Has coverage regressed anywhere it was already complete? |
...
| Class    | Files | Meaning                                         |
| -------- | ----- | ----------------------------------------------- |
| Complete | 444   | Pinned by the ratchet at that commit            |
...
`jest.coverage-gate.config.js` holds the 444 paths in two arrays, `PINNED_LOGIC` (logic-carrying
