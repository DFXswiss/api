...
2. Initialisierung von `pep`, `highRisk`, `complexOrgStructure`, `depositLimit` und `amlAccountType`
3. `GwGFileCover`
...
2. Nur nach einem gültigen, abgeschlossenen NameCheck werden noch leere Personal-Risikofelder mit
   den bisherigen GSheet-Werten initialisiert. Vorhandene Werte werden nie überschrieben und die
   Änderung wird im `kyc_log` protokolliert.
3. Die sechs fehlenden PDF-Nachweise werden mit PDFKit erzeugt und über einen eindeutigen
...
Unvollständige oder ungültige JSON-Daten, fehlende Dokumente, offene NameChecks und Storage-Fehler
stoppen den Fall sichtbar. Leere Compliance-Werte werden nicht als `false` interpretiert.
...
- erlaubte Nationalität oder einen abgeschlossenen erforderlichen Aufenthaltstitel
...
Der Endpoint `GET /v1/kyc/admin/dfx-approval/:stepId/status` liefert die aktuellen Blocker für das
Compliance-UI.

## Manuelle Freigabe in Services

Für Personal-Fälle verwendet Services nur noch
`POST /v1/support/:userDataId/dfx-approval`. Die API erzeugt zuerst den Onboarding-Bericht. Erst nach
erfolgreicher Speicherung führt sie Entscheidung, UserData-Änderungen und Audit-Logs atomar aus.
Eine fehlgeschlagene PDF-Erzeugung kann deshalb keinen bereits freigegebenen Teilzustand mehr
hinterlassen. Der bestehende Organisationsablauf bleibt ausserhalb dieser Migration unverändert.
...
2. Services deployen und Statusanzeige sowie manuelle Personal-Entscheidung mit einem Testfall prüfen.
