...
- **452 of the 541 route entries rest on a call graph that is not fully resolved** — a target chosen at runtime, a method reached through inheritance, an entity manager handed into a transaction callback. This does not weaken the `whole rows` group: an unresolved edge can only add load sites, never remove one, so 414 is a lower bound in that direction. In the other direction 411 of them are backed by at least one measured query; the remaining three are the entries discussed below.
...
| GET | 1 |  | `/support/issue/:id/suggestion` | hidden | whole rows | 435 | not yet | yes | `SupportIssueController.getReplySuggestion` | `subdomains/supporting/support-issue/support-issue.controller.ts` |
| POST | 1 |  | `/support/issue/:id/suggestion` | public | whole rows | 435 | not yet | yes | `SupportIssueController.createReplySuggestion` | `subdomains/supporting/support-issue/support-issue.controller.ts` |
| PUT | 1 |  | `/support/issue/:id/suggestion/:messageId/accept` | hidden | whole rows | 435 | not yet | yes | `SupportIssueController.acceptReplySuggestion` | `subdomains/supporting/support-issue/support-issue.controller.ts` |
| PUT | 1 |  | `/support/issue/:id/suggestion/:messageId/reject` | hidden | whole rows | 435 | not yet | yes | `SupportIssueController.rejectReplySuggestion` | `subdomains/supporting/support-issue/support-issue.controller.ts` |
